...
import FocusModeOverlay from '@/components/shared/FocusModeOverlay'; // Import new FocusModeOverlay
import { AnimatePresence } from 'framer-motion'; // For FocusModeOverlay animation
...
export default function Home() {
    ...
    const [error, setError] = useState<string | null>(null);
     ...
     const fetchSectionData = useCallback(async (
        ...
        try {
            ...
            } else if (response && 'animes' in response && response.animes === null && 'mangas' in response && response.mangas === null) {
                 console.warn(`[Home Page] Jikan API returned null for both animes and mangas for ${sectionKey}. Response:`, response);
                 setData([]);
            }
            else {
    ...
    const fetchInitialData = useCallback(async () => {
        setError(null);
        // Jikan parameters: genre, year, minScore, search, status, page, sort, limit
        ...
        try {
            ...
            setNamiPicks([]);
            // setError(prev => prev || "Could not load Nami's Picks."); // Nami picks are non-critical for page load
        } finally {
    ...
    const handleExitFocusMode = useCallback(() => {
        ...
        // Delay clearing focusedItem to allow for exit animation
    ...
    const renderHorizontalSection = (
        ...
        itemComponent: React.FC<{ item: DisplayItem, onEngageFocusMode?: (item: DisplayItem) => void }> = ItemCard, // Updated prop
                 ...
                 <div className="relative">
                  <div className={cn(
                    ...
                    {isLoading && items.length === 0
                        ...
                        : items.length > 0
                            ? items.map((item, index) => ( item && item.id ? React.createElement(itemComponent, { key: `${item.type}-${item.id}-${index}`, item: item, onEngageFocusMode: handleEngageFocusMode }) : null )) // Pass handler
