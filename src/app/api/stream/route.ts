 ...
 *
 * Query Parameters:
 * - source: The provider name (e.g., 'AnimeSuge', 'AniWave') - REQUIRED
 * - link: The URL to the specific episode page on the provider's site - REQUIRED
 * - number: (Optional) The episode number for logging
 * - id: (Optional) The unique episode ID generated by the scraping layer, may be needed by some providers
 *
...
export async function GET(request: Request) {
    ...
    const episodeId = searchParams.get('id'); // Get episode ID if provided
    ...
    // Validate source parameter (optional but recommended)
    const validSources = ['AnimeSuge', 'AniWave', 'AnimeDao']; // Add more as needed
    if (!validSources.includes(source)) {
        console.error(`[API/stream] [${timestamp}] Bad Request: Invalid source specified: ${source}.`);
        return NextResponse.json({ message: `Invalid source parameter: ${source}` }, { status: 400 });
    }

    console.log(`[API/stream] [${timestamp}] Received request - Source: ${source}, Episode Link: ${episodeLink}, Episode Num: ${episodeNumber}, Episode ID: ${episodeId}`);

    try {
        // 1. Get the initial stream link/iframe URL using the centralized service
        // Pass all potentially needed info to fetchStreamingLinks
        const initialStreamLink = await fetchStreamingLinks({
             source,
             link: episodeLink,
             number: episodeNumber ?? undefined, // Pass number if available
             id: episodeId ?? undefined // Pass ID if available
        });
        ...
        if (!resolvedData || !resolvedData.url) {
            ...
            // Fallback: Return the unresolved URL but flag it
        ...
        console.error(`[API/stream] [${timestamp}] Error processing stream request for ${source} - ${episodeLink}:`, err.message);
        // console.error(`[API/stream] Error Stack:`, err.stack); // Optional: Log full stack
        // Provide a generic server error message
        return NextResponse.json({ message: 'Internal server error while fetching stream.', error: err.message }, { status: 500 });
    }
}
