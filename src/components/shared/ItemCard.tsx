...
import { Star, CalendarDays, Tv, BookText, ArrowRight, Film, Layers, Info } from 'lucide-react'; // Added Info
...
interface ItemCardProps {
  ...
  className?: string;
  viewMode?: 'grid' | 'list';
  onScanDna?: (itemId: number) => void; // New prop
}

export const ItemCard: React.FC<ItemCardProps> = ({ item, className, viewMode = 'grid', onScanDna }) => {
  if (!item || typeof item.id !== 'number') return null;
  const linkHref = `/${item.type}/${item.id}`;

  if (viewMode === 'list') {
    return (
      <div className={cn("block group w-full", className)}> {/* No Link here, Card is clickable */}
        <Card className="overflow-hidden glass-deep neon-glow-subtle-hover transition-all duration-300 ease-in-out group-hover:border-primary/40 h-28 flex flex-row rounded-lg shadow-md cursor-pointer"
           onClick={() => window.location.href = linkHref} // Simple navigation
        >
          ...
          {item.type === 'anime' && onScanDna && (
            <Button
                variant="ghost"
                size="icon"
                className="absolute top-1 right-1 h-7 w-7 text-muted-foreground hover:text-primary z-20 bg-card/50 backdrop-blur-sm hover:bg-card/70"
                onClick={(e) => { e.stopPropagation(); onScanDna(item.id); }}
  ...
  // Default Grid View
  return (
    <div className={cn("block w-36 sm:w-40 md:w-44 lg:w-48 flex-shrink-0 h-full snap-start group relative", className)}> {/* Added relative for button positioning */}
      <Link href={linkHref} passHref legacyBehavior>
       ...
       {item.type === 'anime' && onScanDna && (
        <Button
            variant="ghost"
            size="icon"
            className="absolute top-1 left-1 h-7 w-7 text-muted-foreground hover:text-primary z-20 bg-card/60 backdrop-blur-sm hover:bg-card/80 rounded-full"
...
interface BannerCardProps {
  item: DisplayItem;
  className?: string;
  onScanDna?: (animeId: number) => void; // New prop
}

export const BannerCard: React.FC<BannerCardProps> = ({ item, className, onScanDna }) => {
  if (!item || typeof item.id !== 'number') return null;
  const linkHref = `/${item.type}/${item.id}`;
  ...
  return (
    <div className={cn("block w-[88vw] sm:w-[80vw] md:w-[70vw] lg:w-[60vw] xl:w-[50vw] flex-shrink-0 relative aspect-video sm:aspect-[16/7] md:aspect-[16/6] h-auto snap-center group rounded-xl overflow-hidden", className)}>
      <Link href={linkHref} passHref legacyBehavior>
      ...
      {item.type === 'anime' && onScanDna && (
          <Button
            variant="ghost"
            size="icon"
            className="absolute top-3 left-3 h-8 w-8 text-primary-foreground hover:text-primary z-20 bg-black/40 backdrop-blur-sm hover:bg-black/60 rounded-full shadow-lg"
            onClick={(e) => { e.stopPropagation(); e.preventDefault(); onScanDna(item.id); }}
            title="Scan Anime DNA"
          >
            <Info size={16} />
